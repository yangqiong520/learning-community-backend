...
def toggle_like_courseware(courseware_id):
    try:
        ...
        db.session.rollback()
        print(f"Error in toggle_like_courseware: {str(e)}", flush=True)
        import traceback
        traceback.print_exc()
        return error_response(f'操作失败: {str(e)}')

@courseware_bp.route('/favorites', methods=['GET'])
@token_required
def get_courseware_favorites():
    """获取当前用户收藏的教案课件列表"""
    try:
        page = request.args.get('page', 1, type=int)
        per_page = request.args.get('per_page', 12, type=int)
        
        # 查询用户收藏的教案课件，按收藏时间倒序，支持分页
        query = Like.query.filter(
            Like.user_id == request.current_user_id,
            Like.courseware_id != None
        )
        ...
        for like in likes.items:
            if like.courseware_id:
                courseware = Courseware.query.get(like.courseware_id)
                if courseware:
                    item = courseware.to_dict(request.current_user_id)
                    # 添加收藏时间
                    if like.created_at:
